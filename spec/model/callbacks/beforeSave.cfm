...
<describe hint="model.beforeSave">

  <before><cfscript>
    model = createObject("component", "muon.model.Base");
    model = stub("model");
    model.setTransfer(model);
  ...
  <it should="validate, run the callback, then save"><cfscript>
    model.beforeSave("myCallback");
    s = sequence("callbacks");
    $(model).expects("").inSequence(s);
    $(model).expects("_runSaves").inSequence(s);
