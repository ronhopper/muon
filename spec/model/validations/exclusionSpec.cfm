...
<describe hint="model.validatesExclusionOf">
  ...
  <describe hint="(with no options)">
    ...
    <before><cfscript>
      model.validatesExclusionOf("status", "good,bad,ugly");
    </cfscript></before>

    <it should="not add any errors if the status is not in the list"><cfscript>
    ...
    <it should="add the default error message if the status is in the list"><cfscript>
      $(model).stubs("getStatus").returns("bad");
      model.muonRunValidations();
      $(model).errors().on("status").shouldEqual("is included in the list");
    </cfscript></it>
  ...
  <describe hint="(with an array of options)">
    ...
    <before><cfscript>
      ...
      model.validatesExclusionOf("status", options);
    </cfscript></before>
  ...
  <describe hint="(with a custom message)">
    ...
    <before><cfscript>
      model.validatesExclusionOf("status", "good,bad,ugly", "must be a good ##this.getClassName()##");
  ...
  <describe hint="(on create)">
    ...
    <before><cfscript>
      model.validatesExclusionOf(property="status", with="good,bad,ugly", on="create");
      $(model).stubs("getStatus").returns("bad");
    </cfscript></before>
  ...
  <describe hint="(on update)">
    ...
    <before><cfscript>
      model.validatesExclusionOf(property="status", with="good,bad,ugly", on="update");
      $(model).stubs("getStatus").returns("bad");
    </cfscript></before>
  ...
  <describe hint="(conditionally)">
    ...
    <before><cfscript>
      model.validatesExclusionOf(property="status", with="good,bad,ugly", condition="1 + 1 eq ##this.getResult()##");
      $(model).stubs("getStatus").returns("bad");
    </cfscript></before>
  ...
  <describe hint="(multiple properties)">
    ...
    <before><cfscript>
      model.validatesExclusionOf(properties="s1,s2,s3,s4", with="good,bad,ugly");
    ...
    <it should="not add errors to valid properties"><cfscript>
      model.muonRunValidations();
      $(model).errors().on("s3").shouldBeEmpty();
      $(model).errors().on("s4").shouldBeEmpty();
    </cfscript></it>

    <it should="add errors to each invalid property"><cfscript>
      model.muonRunValidations();
      $(model).errors().on("s1").shouldNotBeEmpty();
      $(model).errors().on("s2").shouldNotBeEmpty();
    </cfscript></it>
